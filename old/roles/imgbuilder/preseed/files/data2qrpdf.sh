...
encodes it in base32 and generates one ore more alphanumeric qrcodes,
and put the resulting qrcodes arranged om a 2x2 matrix inside a pdf.
 ...
 * manually linked QRCode, Version 29, 100 Codes Maximum (4 Codes per A4 Page, 25 Pages Maximum)
...
data2pdf() {
  ...
  if test $fsize -le 2110; then
      cat $fname | 
      python -c "import sys, base64; sys.stdout.write(base64.b32encode(sys.stdin.read()))" |
      qrencode -o $tempdir/$fbase.png -l M -i
  else
      cat $fname |
      python -c "import sys, base64; sys.stdout.write(base64.b32encode(sys.stdin.read()))" |
      split -a 2 -b 1826 -d - $tempdir/$fbase-
