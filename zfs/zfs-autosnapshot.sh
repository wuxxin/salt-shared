...
print_log() { # level, message, ...
	...
	inf*)
		...
		test -z "$opt_quiet" && test "$opt_verbose" -gt 0 && echo $*
		;;
	deb*)
...
ZPOOL_STATUS=$(env LC_ALL=C zpool status 2>&1) ||
	{
		print_log error "zpool status $?: $ZPOOL_STATUS"
		exit 135
	}
...
ZFS_LIST=$(env LC_ALL=C zfs list -H -t filesystem,volume -s name \
	...
	{
		print_log error "zfs list $?: $ZFS_LIST"
		exit 136
	}
...
if [ -n "$opt_fast_zfs_list" ]; then
	...
	if [ -n "$opt_label" ]; then
		SNAPSHOTS_OLD=$(env LC_ALL=C zfs list -H -t snapshot -o name -s name |
			...
			{
				print_log error "zfs list $?: $SNAPSHOTS_OLD"
				exit 137
			}
	else
		SNAPSHOTS_OLD=$(env LC_ALL=C zfs list -H -t snapshot -o name -s name |
...
for ii in $CANDIDATES; do
	...
	for jj in "$@"; do
		...
		if [ "$jj" = '//' -o "$jj" = "$ii" ]; then
			IN_ARGS=$(($IN_ARGS + 1))
		elif [ -n "$opt_recursive" -a "$iii" != "${iii#$jjj}" ]; then
			IN_ARGS=$(($IN_ARGS + 1))
		fi
	done
