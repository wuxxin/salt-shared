...
def crc_matrix_apply(m, value=0):
    return (
        matrix_mul_vector(m, (value ^ 0xFFFFFFFF) | (1 << 32)) & 0xFFFFFFFF
    ) ^ 0xFFFFFFFF
...
def code_for_length(length):
    ...
    return (
        base_code + ((length - base_length) >> num_bits),
        (length - base_length) & ((1 << num_bits) - 1),
        num_bits,
    )
...
def bulk_deflate(
    repeated_byte, compressed_size=None, max_uncompressed_size=None, final=1
):
    assert (compressed_size is None and max_uncompressed_size is not None) or (
        compressed_size is not None and max_uncompressed_size is None
    )
    ...
    num_code_length_codes = (
        max(CODE_LENGTH_ALPHABET.index(sym) for sym in code_length_lengths) + 1
    )
    ...
    def skip(n):
        while n >= 11:
            if n < 138:
                ...
                n < 138 + 11
                and code_length_lengths[18] < (n - 138) * code_length_lengths[0]
            ):
    ...
    if max_uncompressed_size is not None:
        ...
        while (
            bits.bit_pos + ll_lengths[285] + distance_lengths[0] + ll_lengths[256] <= 8
        ):
            bits.push_rev(*ll_codes[285])
            bits.push(*distance_codes[0])
            n += 258
...
def bulk_bzip2(repeated_byte, compressed_size=None, max_uncompressed_size=None):
    ...
    compressed_data = (
        header + block * num_blocks + footer + struct.pack(">L", stream_crc)
    )
...
class CentralDirectoryHeader:
    ...
    def serialize(self, zip64=False):
        ...
        assert (
            self.local_file_header_offset <= 0xFFFFFFFE
        ), self.local_file_header_offset
    ...
    def serialize(self, zip64=False, compression_method=COMPRESSION_METHOD_DEFLATE):
        ...
        result.append(
            struct.pack(
                ...
                zip64
                and 0xFFFF
                or self.cd_num_entries,  # number of central directory entries on this disk
                zip64
                and 0xFFFF
                or self.cd_num_entries,  # number of central directory entries total
...
def write_zip_no_overlap(
    ...
    **kwargs
    ...
    kernel, n, crc_matrix = BULK_COMPRESS[compression_method](
        CHOSEN_BYTE,
    ...
    for i in range(num_files):
        file = LocalFileHeader(
            len(kernel),
            n,
            crc_matrix_apply(crc_matrix),
            filename_for_index(i),
    ...
    offset += f.write(
        EndOfCentralDirectory(len(central_directory), cd_size, cd_offset).serialize(
            zip64=zip64, compression_method=compression_method
        )
    )
    ...
    **kwargs
    ...
    offset += f.write(
        EndOfCentralDirectory(
            len(central_directory) + num_files, cd_size, cd_offset
        ).serialize(compression_method=compression_method, zip64=zip64)
    )
...
def crc_combine(crc_prefix, crc_remainder, crc_matrix_zeroes):
    ...
    return (
        crc_matrix_apply(crc_matrix_zeroes, crc_prefix ^ 0xFFFFFFFF)
        ^ 0xFFFFFFFF
        ^ crc_remainder
    )


def write_zip_quoted_overlap(
    f,
    num_files,
    compressed_size=None,
    max_uncompressed_size=None,
    ...
    if extra_tag is not None:
        ...
        while sum <= 65535:
            sum += (
                ...
                + 4
                + (zip64 and 20 or 0)
                + len(filename_for_index(num_extra_length_files + 1))
            )
            num_extra_length_files += 1
        num_extra_length_files -= 1

    # Compress the kernel.
    kernel, kernel_uncompressed_size, kernel_crc_matrix = BULK_COMPRESS[
        compression_method
    ](
        CHOSEN_BYTE,
        compressed_size=compressed_size,
    ...
    header = LocalFileHeader(
        ...
        crc_matrix_apply(kernel_crc_matrix),
        filename_for_index(num_files - 1),
        compression_method=compression_method,
    )
    ...
    while (
        compression_method == COMPRESSION_METHOD_DEFLATE
        and len(files) < num_files - num_extra_length_files
    ):
        ...
        new_crc_matrix = matrix_mul(
            next_file.crc_matrix, memo_crc_matrix_repeated(b"\x00", num_quoted)
        )
    ...
    while len(files) < num_files:
        ...
        header = LocalFileHeader(
            ...
            compression_method=compression_method,
            extra_tag=extra_tag,
            extra_length_excess=len(next_header_bytes)
            + next_file.header.extra_length_excess,
        )
    ...
    offset += f.write(
        EndOfCentralDirectory(len(central_directory), cd_size, cd_offset).serialize(
            compression_method=compression_method, zip64=zip64
        )
    )
...
def main():
    ...
    if giant_steps and mode != write_zip_quoted_overlap:
        print(
            "--giant-steps only makes sense with --mode=quoted_overlap", file=sys.stderr
        )
        sys.exit(1)
    max_quoted = 0
    if giant_steps:
