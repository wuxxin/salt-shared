prepare_backup () {
    # create ready to use /app/.gnupg for backup being done using duplicity
    mkdir -p /app/.gnupg
    find /app/.gnupg -mindepth 1 -name "*.gpg*" -delete
    echo "$APPLIANCE_BACKUP_ENCRYPT" | gosu app gpg --homedir /app/.gnupg --batch --yes --import --
    # write out backup target and gpg_key to duply config
    gpg_key_id=$(gosu app gpg --keyid-format 0xshort --list-key ecs_backup | grep pub | sed -r "s/pub.+0x([0-9A-F]+).+/\1/g")
    cat /app/.duply/appliance-backup/conf.template | \
        ...
        /app/.duply/appliance-backup/conf
    chown app:app /app/.duply/appliance-backup/conf
}
