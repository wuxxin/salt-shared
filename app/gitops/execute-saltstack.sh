...
salt_install() { # $1=salt_from_ppa(true|false)
    ...
    if [[ $os_distributor =~ ^(debian|ubuntu)$ ]]; then
        ...
        DEBIAN_FRONTEND=noninteractive apt-get install -y \
            curl git gnupg git-crypt salt-minion python3-pytoml
        ...
        pamac install --no-confirm --no-upgrade curl git gnupg git-crypt salt python-pygit2 python-looseversion
    elif test "$os_distributor" = "arch"; then
        pacman -Sy curl git gnupg git-crypt salt python-pygit2
    fi
}
...
main() {
    ...
    from_ppa="false"
    if test "$1" = "--from-ppa"; then
        from_ppa="true"
        shift
    fi
    if test "$1" = "--etc-dir"; then
        config_path="$2"
        shift 2
    fi
