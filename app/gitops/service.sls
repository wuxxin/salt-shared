...
/usr/local/lib/gitops-library.sh:
  file.managed:
    - source: salt://app/gitops/gitops-library.sh
    - template: jinja
    - defaults:
        settings: {{ settings }}
    - require:
...
/usr/local/sbin/gitops-update.sh:
  file.managed:
    - source: salt://app/gitops/gitops-update.sh
    - mode: "0755"
    - template: jinja
    - defaults:
        settings: {{ settings }}
...
/etc/systemd/system/gitops-service-failed@.service:
  file.managed:
    - source: salt://app/gitops/gitops-service-failed@.service
    - template: jinja
    - defaults:
      settings: {{ settings }}
...
gitops-update:
  file.managed:
    ...
    - source: salt://app/gitops/gitops-update.service
    - template: jinja
    - defaults:
        settings: {{ settings }}
  cmd.run:
