{% from "app/backup/defaults.jinja" import settings with context %}
...
/usr/local/sbin/backup-run.sh:
  file.managed:
    - source: salt://app/backup/backup-run.sh
    - template: jinja
    - defaults:
        settings: {{ settings }}
    - mode: "0755"
...
/usr/local/lib/backup-service.sh:
  file.managed:
    - source: salt://app/backup/backup-service.sh
    - template: jinja
    - defaults:
        settings: {{ settings }}
    - mode: "0755"
...
/etc/systemd/system/backup.timer:
  file.managed:
    - source: salt://app/backup/backup.timer
    - template: jinja
    - defaults:
        settings: {{ settings }}
    - require:
