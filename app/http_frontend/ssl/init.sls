...
{% from "app/http_frontend/ssl/lib.sls" import deploy_from_file, deploy_from_pillar,
...
/usr/local/bin/create-selfsigned-host-cert.sh:
  file.managed:
    - mode: "0755"
    - source: salt://app/http_frontend/ssl/create-selfsigned-host-cert.sh
    - template: jinja
    - defaults:
        settings: {{ settings }}
...
{{ settings.ssl.base_dir }}/ssl-renew-hook.sh:
  file.managed:
    - source: salt://app/http_frontend/ssl/ssl-renew-hook.sh
    - mode: "0755"
    - template: jinja
