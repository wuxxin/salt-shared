# -*- coding: utf-8 -*-
'''
Packer Vagrant Libvirt KVM Cloud Module
========================================

cloud configuration at ``/etc/salt/cloud.providers`` or
``/etc/salt/cloud.providers.d/packer-vagrant-libvirt.conf``:

.. code-block:: yaml

    my-packer-vagrant-libvirt-config:
...
def __virtual__():
    '''
    Check for Packer Vagrant Libvirt configurations
    '''
    if get_configured_provider() is False:
        log.debug(
            'There is no Packer Vagrant Libvirt cloud provider configuration '
            'available. Not loading module.'
        )
        return False

    log.debug('Loading Packer Vagrant Libvirt cloud module')
    return True
...
def get_configured_provider():
    ...
    return config.is_provider_configured(
        __opts__,
        __active_provider_name__ or 'packer-vagrant-libvirt',
        ('api_key',)
    )
...
def create(vm_):
    ...
    try:
        ...
        log.error(
            'Error creating {0} on packer-vagrant-libvirt\n\n'
...
def query(method='droplets', droplet_id=None, command=None, args=None):
    '''
    Make a web call to Packer Vagrant Libvirt
    '''
    ...
    if request.getcode() != 200:
        raise SaltCloudSystemExit(
            'An error occurred while querying Packer Vagrant Libvirt. HTTP Code: {0}  '
            'Error: {1!r}'.format(
                request.getcode(),
                request.read()
            )
...
def show_instance(name, call=None):
    '''
    Show the details from Packer Vagrant Libvirt concerning a droplet
    '''
    if call != 'action':
        raise SaltCloudSystemExit(
